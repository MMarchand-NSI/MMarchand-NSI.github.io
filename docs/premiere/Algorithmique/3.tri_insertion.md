...
!!! note "Rappel d'ouverture (5 minutes, cours fermé)"
    On répond **sans rouvrir** la page sur le tri par sélection.

    1. Énoncer en une phrase le principe du tri par sélection.
    2. Après trois étapes du tri par sélection, que peut-on affirmer sur les trois premières cases du tableau ?
    3. Combien de comparaisons ce tri effectue-t-il au total sur un tableau de taille $n = 5$ ?

    ??? success "Corrigé"
        1. À chaque étape, on cherche le **minimum de la partie non triée** et on l'échange avec la première case de cette partie.
        2. Elles contiennent les **trois plus petits éléments** du tableau, **dans l'ordre croissant**, et ils ne bougeront plus.
        3. $5 + 4 + 3 + 2 = 14$ comparaisons. Le total général est $\dfrac{n(n+1)}{2}$, donc de l'ordre de $n^2$.
