...
!!! note "Rappel d'ouverture (5 minutes, cours fermé)"
    On répond **sans rouvrir** les pages sur les tris.

    1. Pour chercher une valeur dans un tableau **quelconque** de $n$ éléments, combien de comparaisons faut-il au pire ? Quelle complexité ?
    2. Qu'est-ce qui est vrai du **début** du tableau à chaque étape du tri par insertion ?
    3. Combien d'entiers contient l'intervalle semi-ouvert $[3 ; 7)$ ? Lesquels ?

    ??? success "Corrigé"
        1. $n$ comparaisons au pire (la valeur est en dernier, ou absente) : c'est du $\mathcal{O}(n)$, linéaire. Il faut tout parcourir, faute de savoir où chercher.
        2. Le début du tableau est **déjà trié** ; on y insère l'élément suivant à sa place. C'est ce qui distingue ce tri de la sélection, où le début est trié **et définitif**.
        3. Quatre entiers : $3, 4, 5, 6$. La borne de gauche est incluse, celle de droite exclue. Cette convention est celle de toute la page qui suit, et c'est elle qui rend l'algorithme juste du premier coup.
...
!!! warning "Ne pas chercher d'autres versions en ligne"
    La recherche dichotomique est un algorithme classique, mais les implémentations que l'on trouve en ligne sont **très hétérogènes** : bornes incluses ou exclues, `fin = milieu` ou `fin = milieu - 1`, conditions d'arrêt différentes. Plusieurs de ces variantes sont correctes **prises isolément**, mais on ne peut pas en prendre un morceau et le recoller à une autre : c'est ainsi qu'apparaissent les bugs sur les cas limites (tableau vide, élément absent, dernier élément). Wikipédia n'y échappe pas, et une IA vous répondra elle aussi dans une convention qui n'est pas forcément celle du cours.

    Restez donc sur la version présentée ici, en intervalles semi-ouverts `[a ; b)` : elle est correcte par construction, et surtout c'est **la seule dont vous tenez l'invariant**. Tant que vous ne savez pas énoncer l'invariant d'une variante, vous n'avez aucun moyen de dire si elle est juste : c'est précisément ce que ce chapitre vous apprend à faire.
