...
"Au début de l'itération $i$, $mini$ contient le minimum de $T[0..i)$"
...
!!! hint "Notations"
    ...
    - $T[i..j)$ représente le sous-tableau de $T$ de l'indice $i$ à l'indice $j$ exclus.
    Parc onséquent:
        - $T[i..i+1)$ représente le sous-tableau de $T$ ne comportant que $T[i]$.
        - Pour un tableau $T$ de taille $n$, $T=T[0..n)$
        - Quel que soit $i$, $T[i..i)$ est le tableau vide
...
2. Si le tableau a au moins un élément, $i=1$ et $mini$ est bien le minimum de $T[0..1)$, le tableau constitué d'un seul élément.
...
Donc en début d'itération $i$, "$mini$ est le minimum de $T[0..i)$" est vrai

On veut alors prouver qu'après une itération, "$mini$ est le minimum de $T[0..i)$" est vrai

Déroulons l'algorithme:

- Si $T[i] \gt mini$, alors $mini$ est aussi le $minimum$ de $T[0..i+1)$ et l'invariant est vérifié en sortie d'itération, car on ne fait rien dans ce cas.
- Sinon, $T[i] \le mini$, et donc $mini$ n'est pas le minimum de $T[0..i+1)$. Mais nous rétablissons immédiatement l'invariant en affectant $T[i]$ à $mini$ qui devient bien le minimum de $T[0..i+1)$.

Dans tous les cas, en sortie de boucle, "$mini$ est le minimum de $T[0..i+1)$" est vrai, ce que nous voulions démontrer.
...
Donc mini est le minimum de $T[0..n)$
...
```markdown
# Algorithme - Calculer le minimum de T[0..n)
1. mini ← T[0]
2. Pour i allant de 1 à n-1:
3.     Si T[i] < e
4.          mini ← T[i]
...
Ce qui nous indique que le prouveur a déterminé que notre algo est totalement correct.
