...
!!! note "Rappel d'ouverture (5 minutes, cours fermé)"
    ...
    1. Après `a = 4` puis `b = a` puis `a = 7`, que valent `a` et `b` ?
    2. Avec `s = "chat"`, que se passe-t-il si on écrit `s[0] = "p"` ?
    3. Écris la boucle qui construit la chaîne `mot` à l'envers dans une variable `res`.

    ??? success "Corrigé"
        1. `a` vaut `7` et `b` vaut `4`. `b = a` a **copié la valeur** au moment de l'affectation ; il n'a pas créé de lien durable entre les deux.
    ...
    La première question est celle qui compte aujourd'hui. Tu vas voir que sur une **liste**, la même écriture `b = a` ne se comporte pas du tout de la même façon, et c'est le point le plus délicat de cette page.
...
Tu remarqueras qu'on ne peut pas utiliser les simples guillemets pour s4.
...
!!! abstract "Attention"
    Dès que tu utiliseras `=` directement sur la variable, le même mécanisme que précédemment s'applique. Un nouvel emplacement mémoire sera créé, donc tu perdras l'avantage de la mutabilité.
...
### Erreur 5 : utiliser `len(seq)` comme indice
