...
*Je suis professeur en sciences informatiques. J'utilise la théorie des catégories dans mes cours sur les types structurés et la récursivité, suffisamment pour mes besoins, sans prétendre la maîtriser complètement. Elle m'aide à organiser de grandes classes de programmes (catamorphismes, anamorphismes, hylomorphismes, transformations structurales) et à justifier des choix qui peuvent sembler arbitraires, comme l'emploi des intervalles semi-ouverts, qui simplifie les algorithmes en éliminant les cas à la marge, et la théorie des catégories en garantit la cohérence et la naturalité. Mes élèves ne voient pas ce cadre directement, mais il structure ce que je leur enseigne.*

*Un jour, un collègue professeur de philosophie est tombé sur un article dans le magazine Epsiloon de mai 2026 consacré à la théorie des catégories. Intrigué, il m'a posé des questions. J'ai essayé de répondre, et je me suis retrouvé bloqué assez vite : tout ce qui existe comme introduction demande de connaître des structures mathématiques avancées que mon interlocuteur ne connaissait pas, et que je ne pouvais pas expliquer en quelques minutes de conversation.*

*Ce texte est ma tentative de combler ce manque. Je ne suis pas philosophe, et ça se verra sûrement. Mais les exemples que j'ai choisis viennent du terrain commun que je cherchais ce jour-là.*
...
À un moment, certains mathématiciens ont commencé à remarquer quelque chose d'un peu dérangeant. En topologie et en algèbre par exemple, la façon de passer d'un objet géométrique à un objet algébrique associé semblait suivre toujours les mêmes règles, quel que soit l'objet de départ. Et plus généralement, les mêmes démonstrations réapparaissaient dans des domaines complètement distincts. Pas des démonstrations similaires, mais les mêmes, mot pour mot, à ceci près que les objets portaient des noms différents.
...
> *Mon collègue m'a aussi demandé quel était le lien avec [John von Neumann](https://fr.wikipedia.org/wiki/John_von_Neumann), figure incontournable des mathématiques et de l'informatique de cette époque. La réponse est simple : ailleurs. Von Neumann travaillait sur un programme très différent, basé sur la théorie des ensembles, le calcul numérique et les automates. Il n'a pas contribué à la théorie des catégories et ne semble pas l'avoir utilisée. Ce n'est pas un oubli : les deux démarches partent dans des directions opposées. Von Neumann cherchait à construire et à calculer ; Eilenberg et Mac Lane cherchaient à abstraire et à comparer. C'est justement cette différence qui rend la théorie des catégories originale.*
...
Ce déplacement, des objets vers les relations, c'est de là que tout part.
...
Une catégorie, c'est des **objets**, des **flèches** (qu'on appelle aussi *morphismes*) entre ces objets, et deux règles que ces flèches doivent respecter.
...
Dans notre exemple, chaque concept est inclus dans lui-même. Un labrador est un labrador. Cette flèche existe bien.

Les deux règles sont satisfaites, donc nous avons bien une catégorie.

Cette définition est d'une sobriété presque déconcertante. Mais c'est justement cette sobriété qui lui donne sa force, car en n'exigeant rien de plus, elle s'applique à une quantité extraordinaire de situations différentes. Dès qu'on a des objets et des relations entre eux qui se composent, on a une catégorie.
...
De P vers Q, on peut raisonner directement, passer par des étapes intermédiaires, ou faire appel à d'autres hypothèses. Ce sont des flèches distinctes entre les mêmes objets.
...
La première regarde vers l'intérieur : deux choses sont identiques si elles partagent les mêmes propriétés intrinsèques, soit la même composition, la même nature, le même contenu.
...
Ces deux réponses ne coïncident pas toujours. C'est la seconde que la théorie des catégories formalise, avec la notion d'**isomorphisme**.
...
Ce qui est frappant dans cette définition, c'est ce qu'elle **ne dit pas**. Elle ne dit rien sur la nature de A et B. Elle ne demande pas qu'ils soient composés des mêmes éléments, qu'ils aient le même nom, ni même qu'ils aient quoi que ce soit en commun en eux-mêmes. Deux objets sont "le même" au sens catégoriel parce qu'ils se comportent de la même façon par rapport au reste de la catégorie, et pour aucune autre raison.
...
Un foncteur est une traduction d'une catégorie vers une autre qui **préserve la structure** : les objets sont traduits en objets, les flèches en flèches, et les compositions restent des compositions. On ne peut pas traduire n'importe comment : la traduction doit respecter la façon dont les relations sont organisées.
...
D'après ce que j'en comprends, ce serait particulièrement utile pour des questions comme : est-ce que deux théories disent la même chose avec des mots différents ? Qu'est-ce qu'on perd ou qu'on préserve quand on passe d'un cadre conceptuel à un autre ? Ce sont en tout cas des questions qui me semblent familières en philosophie, et le foncteur semble y apporter un langage plus précis que l'intuition seule.

En discutant, mon collègue m'a aussi demandé si les monades de Leibniz avaient un rapport avec la théorie des catégories. La question est naturelle : le mot "monade" existe dans les deux univers. En théorie des catégories, une monade est un foncteur particulier qui va d'une catégorie vers elle-même, avec des propriétés supplémentaires. C'est un concept qui devient de plus en plus courant en informatique : explicitement nommé dans les langages fonctionnels comme Haskell, mais présent aussi de façon plus discrète dans d'autres langages, comme Python avec ses décorateurs et wrappers.

Mais le lien avec Leibniz est ténu. La monade catégorielle apparaît en 1958 chez le mathématicien Roger Godement, sous le nom de "construction standard", et le mot "monade" n'est utilisé pour ce concept qu'à partir de 1967. Rien ne prouve que le choix du mot était délibéré. Les deux mots viennent du grec *monas*, qui signifie "unité". D'après les sources que j'ai trouvées, [le lien s'arrête là](https://byorgey.wordpress.com/2009/01/12/abstraction-intuition-and-the-monad-tutorial-fallacy/) : les deux concepts se sont développés indépendamment.
...
La théorie des catégories fait l'objet d'une hype croissante, notamment depuis qu'elle est devenue visible hors des mathématiques. Il vaut la peine de distinguer ce qui est vraiment documenté de ce qui l'est moins.
...
En informatique théorique, l'influence est directe et concrète. La correspondance de Curry-Howard-Lambek, nommée d'après trois chercheurs qui ont découvert indépendamment le même résultat, établit que les types dans un langage de programmation, les propositions logiques et les objets d'une catégorie sont trois façons de décrire la même chose. Ce n'est pas une métaphore, mais un résultat précis qui sous-tend la conception des langages fonctionnels modernes.
...
La tentation existe de voir des catégories partout dès qu'il y a des relations entre des choses. Ce n'est pas faux en soi : techniquement, n'importe quelle collection d'objets reliés par des relations qui se composent forme une catégorie. Mais dire ça n'apporte rien si le cadre n'interdit rien, ne prédit rien de non-évident, et ne permet pas de transporter des résultats d'un domaine à l'autre. Un cadre utile travaille : il force à des conclusions qu'on n'aurait pas vues autrement. Un cadre qui se contente d'habiller ce qu'on savait déjà décore, il n'explique pas.

Il faut aussi dire que l'informatique théorique est devenue centrale dans les sciences, et qu'elle a tiré la théorie des catégories avec elle. Ce n'est pas que la théorie ait changé, c'est que le monde a bougé vers elle.
