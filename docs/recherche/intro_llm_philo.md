...
Cette même conversation nous a amenés à une question connexe, plus inconfortable : est-ce qu'il y a une forme d'arrogance dans le refus de se servir de ces outils ? Pas une arrogance naïve, mais quelque chose de plus subtil : l'idée que la valeur d'un travail intellectuel tiendrait à la pureté de ses outils plutôt qu'à la qualité de sa pensée. Que se targuer de ne pas utiliser un LLM pourrait être une façon de signaler une appartenance, une posture, plutôt qu'une position réfléchie. Si on croit que la valeur est dans ce qu'on provoque et non dans ce dont on est la trace, alors le refus ostentatoire des outils devient difficile à justifier. On n'a pas tranché. Mais la question nous a semblé valoir la peine d'être posée.
