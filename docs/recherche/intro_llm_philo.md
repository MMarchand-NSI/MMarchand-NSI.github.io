...
Un LLM est fondamentalement un programme conçu pour répondre à une question unique : **étant donné ce morceau de texte, quel token devrait venir ensuite ?**
...
**Un LLM n'a pas d'intentionnalité.** Quand il écrit "je pense que" ou "je crois que", il produit le texte qui suit statistiquement une telle formulation dans ses données d'entraînement. Ce n'est pas une affirmation sur un état interne. Ce n'est pas non plus la preuve qu'il n'en a pas : c'est simplement une information dont on ne peut rien tirer dans un sens comme dans l'autre, précisément parce que la boîte noire résiste.
