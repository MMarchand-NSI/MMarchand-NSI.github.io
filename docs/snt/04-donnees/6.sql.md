...
!!! note "Le `WHERE`, c'est la condition que tu connais"
    Troisième rencontre avec la même idée, dans le troisième outil de l'année :

    | Où | Comment ça s'écrit | Sur quoi porte le test |
    |---|---|---|
    | Python (le seuil) | `if luminosite < 128:` | chaque **pixel** de l'image |
    | Tableur (filtre, `NB.SI`) | condition sur une colonne | chaque **cellule** de la plage |
    | SQL | `WHERE age < 18` | chaque **ligne** de la table |

    Toujours la même forme : **une valeur, un test, deux issues**. Ce qui change d'un outil à l'autre, ce n'est pas l'idée, c'est ce que l'outil fait de la réponse. Python exécute un bloc ou l'autre, le tableur masque ou affiche, SQL garde ou écarte la ligne.

    Une différence à noter tout de même : en Python, tu écrivais explicitement le `else`. En SQL, il n'y en a pas, parce qu'il est **sous-entendu** : ce qui ne vérifie pas la condition n'apparaît simplement pas dans le résultat.
