...
!!! note "Tu connais déjà les conditions"
    Le mot **condition** n'est pas nouveau. Au moment du seuil, en Python, tu écrivais :

    ```python
    if luminosite < 128:
        # ce pixel devient noir
    else:
        # ce pixel devient blanc
    ```

    C'est la **même idée** : un test qui, pour chaque élément, répond oui ou non, et un traitement différent selon la réponse. Seuls l'outil et l'écriture changent.

    | Où | Comment ça s'écrit | Ce que ça fait |
    |---|---|---|
    | Python (le seuil) | `if luminosite < 128:` | traite chaque **pixel** selon le test |
    | Tableur (le filtre) | condition posée sur une colonne | **masque** les lignes qui échouent au test |
    | Tableur (`NB.SI`) | `"<10"` entre guillemets | **compte** les cellules qui passent le test |

    Retiens la forme commune : **une valeur, un test, deux issues**. Tu la retrouveras encore dans le dernier outil du thème.
