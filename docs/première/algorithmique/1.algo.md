...
!!! abstract "Problème"
    ...
    | SEARCH | Trouver un élément dans une structure (liste, tableau, graphe…). |
    ...
    Une partie importante de la recherche en informatique consiste à trouver des algorithmes de plus en plus efficaces pour résoudre des problèmes donnés. C'est même au coeur de l'algorithmique.
...
!!! abstract "Boucle bornée"
    ...
    **Une boucle bornée dont l'indice de départ est supérieure à son indice d'arrivée ne s'exécute pas**. De même, en mathématiques, $\displaystyle \sum_{k=6}^{2}k=0$ car c'est une somme qui n'accumule aucune valeur. En effet, la condition d'arrêt de la boucle est déjà remplie avant de s'y engager.
...
!!! abstract "Boucle non bornée"
    ...
    On ne peut pas savoir a priori combien de fois elle va s’exécuter : cela dépend d’une condition dynamique, qui évolue pendant l’exécution.
    ...
    ici, la terminaison dépend d'une condition dont on ne peut pas a priori prévoir à l'avance si elle termine.
...
!!! abstract "Invariant de boucle et Correction totale"
...
!!! abstract "Le problème de l'indice minimum - ARGMIN"
...
La preuve de correction partielle à l'aide de l'invariant de boucle est appelé un raisonnement inductif, ou récurrent.
...
Il est important d'initialiser, car il faut que ça soit vrai au moins une fois pour que ça soit "infiniment vrai" par hérédité.
